...
if (!firebaseConfigured()) show('Firebase setup required before creating shared rooms. Paste the web configuration into js/firebase-config.js.', 'error');
...
document.querySelector('#quick-form')?.addEventListener('submit', async event => {
  ...
  const config = { id: roomId, type: 'quick_match', teamA: String(form.get('teamA')).trim(), teamB: String(form.get('teamB')).trim(), rules: normalizeRules(Object.fromEntries(form)), createdAt: Date.now() };
  try {
    ...
    const root = document.querySelector('#created'); root.classList.remove('hidden'); document.querySelector('#quick-links').innerHTML = linksMarkup(links); bindCopies(root); show(`Room ${roomId} created. Save the organizer link.`, 'success'); root.scrollIntoView({ behavior: 'smooth' });
