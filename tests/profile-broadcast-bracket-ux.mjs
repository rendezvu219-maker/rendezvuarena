...
assert.match(dashboardHtml,/dashboard\.css\?v=0\.7\.0-tournament-workflow/);
assert.match(dashboardHtml,/dashboard\.js\?v=0\.7\.0-tournament-workflow/);
...
assert.match(portalJs,/captain-managed/);
assert.match(portalJs,/CAPTAIN · DRAFT CONTROLLER/);
assert.match(portalJs,/ROSTER MEMBER · VIEW ONLY/);
assert.match(portalJs,/CAPTAIN-ONLY DRAFT CONTROL/);
assert.match(portalJs,/\$\{isCaptain\?`<button[\s\S]{0,1000}id="captain-open-draft"/, 'The Draft Room control must stay inside the Captain-only template branch.');
assert.match(dashboard,/START TOURNAMENT & OPEN CHECK-IN/);
assert.match(dashboard,/captainAccountRequirement/);
assert.match(portalJs,/is-own-match/);
assert.match(portalJs,/ops-chat-image/);
assert.match(dashboard,/renderBroadcastMatchModal/);
assert.match(dashboard,/results\/request-reconfirmation/);
assert.match(dashboard,/START TOURNAMENT/);
