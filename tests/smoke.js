...
(async () => {
  ...
  try {
    ...
    hostSocket.socket.emit('draft:state', {
      ...
      state: { status: 'complete', gameNumber: 1, gameRollId: draft.room.config.gameRollId, engine: { state: 'complete', gameNumber: 1, teamA: { picks: ['0001'], bans: ['0005'] }, teamB: { picks: ['0002'], bans: ['0006'] } }, chosenDivineRules: [] },
    });
